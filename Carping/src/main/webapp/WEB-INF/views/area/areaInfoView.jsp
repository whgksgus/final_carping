...
<body>
	...
	var mapData;
	 var mapContainer = document.getElementById('map'), // 지도를 표시할 div  
		...
		kakao.maps.event.addListener(marker, 'click', function() {
			...
			// 마커 위에 인포윈도우를 표시합니다
	      	...
	      	$.ajax({
	    		...
	    		,beforeSend:function(){
	    			/* div 감추기 처리 */
    	        	$('.areaInfo').addClass('display-none');
    	    	}
    	    	,complete:function(){
    	    		/* div 보여주기 */
	    			$('.areaInfo').removeClass('display-none');
	    ...
	    function getFoodZoneList(){
	    	
	    	$.ajax({
	    		url : "selectFoodZoneList.do",
	    		type : "post",
	    		data : {"foodZoneKey" :  positions.title},
	    		success : function(data){
	    ...
	    function getAreaReviewList(){
			$.ajax({
				url : "selectAreaReviewList.do",
	    		type : "post",
	 	...
	 	function getUpdateScoreAvg(){
			$.ajax({
				url : "updateArScoreAvg.do",
	    		type : "post",
	    		data : {"scoreAvg" : scoreAvg},
	    		success : function(data){
	...
	<div class="areaInfo display-none">
	<br>
	<br>
	<div style="margin-left: 5%; width: 90%; border-bottom: 2px solid lightgray;"></div>
	<br>
	<br>
	<div id="image" ><img style="margin-left: 20%; width: 1000px; height:600px; align: center;" src="/resources/images/${area.areaImage }"></div>
	<br>
	<h1 class="h1" align="center">${area.areaName }</h1>
	<br>
	<div align="center" style="display:inline-block; width:70%; height:150px; margin-left:15%;">
		<h3 class="h3" >${area.areaAddress }</h3>
	...
	<div style="width: 70%; height: 800px; margin-left:15%;">
		...
		<div style="width:50%; float:left;display:block;">
			...
			<div style="width: 95%; border-bottom: 2px solid lightgray; display:inline-block;"></div>
		...
		<!-- 리뷰 -->
		<div style="width:50%; float:left;display:block;">
				<h3 class="h3" style="width:45%; float:left;">리뷰</h3><h3 class="h3" style="width:50%; float:left;">평점 : ${area.scoreAvg }</h3>
			<div style="width:100%; border-bottom: 2px solid lightgray; display:inline-block;"></div>
		...
		<div style="width:100%; height:200px; text-align:center; background-color: rgba(12, 12, 12, 0.36); display:inline-block;">
			<br>
			<h3 class="h3">기타 사항</h3>
			<br>
			<span>${area.areaEtc }</span>			
		</div>
	</div>
	</div>
	<br>
